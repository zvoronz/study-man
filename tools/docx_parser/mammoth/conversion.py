...
class _ConversionContext(object):
    ...
    is_symbol = cobble.field(default=None)
...
class _DocumentConverter(documents.element_visitor(args=1)):
    ...
    def visit_run(self, run, context):
        ...
        if run.font != None and run.font == 'Symbol':
            element = html_paths.element(["span"], fresh=False)
            element.tag.attributes.update({"style": "font-family:symbol;"})
            context.is_symbol = True
    ...
    def visit_text(self, text, context):
        return [html.text(text.value, context.is_symbol)]
    ...
    def visit_tab(self, tab, context):
        return [html.text("\t", context.is_symbol)]
    ...
    def visit_note_reference(self, note_reference, context):
        ...
        return [
            html.element("sup", {}, [
                ...
                }, [html.text("[{0}]".format(note_number), context.is_symbol)])
            ])
        ]
    ...
    def visit_note(self, note, context):
        note_body = self._visit_all(note.body, context) + [
            html.collapsible_element("p", {}, [
                html.text(" ", context.is_symbol),
                html.element("a", {"href": "#" + self._note_ref_html_id(note)}, [
                    html.text(_up_arrow, context.is_symbol)
                ]),
            ])
        ]
        return [
    ...
    def visit_comment_reference(self, reference, context):
        def nodes():
            ...
            return [
                ...
                }, [html.text(label, context.is_symbol)])
            ]
    ...
    def visit_comment(self, referenced_comment, context):
        ...
        return [
            html.element(
                ...
                [html.text("Comment {0}".format(label), context.is_symbol)],
            ),
